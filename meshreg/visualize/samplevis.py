...
def sample_vis(sample,
    ...
    for row_idx in range(row_nb):
        ...
        if pred_handjoints2d is not None:
            visualize_joints_2d(axes[row_idx, 0],
                                pred_handjoints2d[row_idx],
                                alpha=1,
                                joint_idxs=False)
        if gt_handjoints2d is not None:
            visualize_joints_2d(axes[row_idx, 0],
                                gt_handjoints2d[row_idx],
                                alpha=0.5,
                                joint_idxs=False)
        ...
        if pred_objverts2d is not None:
            axes[row_idx, 1].scatter(pred_objverts2d[row_idx, :, 0],
                                     pred_objverts2d[row_idx, :, 1],
                                     c="r",
                                     s=1,
                                     alpha=0.2)
        if gt_objverts2d is not None:
            axes[row_idx, 1].scatter(gt_objverts2d[row_idx, :, 0],
                                     gt_objverts2d[row_idx, :, 1],
                                     c="b",
                                     s=1,
                                     alpha=point_alpha)
        if pred_objcorners2d is not None:
            visualize_joints_2d(
                ...
                alpha=1,
                joint_idxs=False,
                links=[[0, 1, 3, 2], [4, 5, 7, 6], [1, 5], [3, 7], [4, 0],
                       [0, 2, 6, 4]],
            )
        ...
        if gt_objverts2d is not None and pred_objverts2d is not None:
            ...
            arrow_nb = len(idxs)
            arrows = torch.cat([
                gt_objverts2d[:, idxs].float(), pred_objverts2d[:,
                                                                idxs].float()
            ], 1)
        ...
        if gt_objverts3dw is not None:
            axes[row_idx, col_idx].scatter(gt_objverts3dw[row_idx, :, 0],
                                           gt_objverts3dw[row_idx, :, 1],
                                           c="b",
                                           s=1,
                                           alpha=point_alpha)
        if pred_objverts3dw is not None:
            axes[row_idx, col_idx].scatter(pred_objverts3dw[row_idx, :, 0],
                                           pred_objverts3dw[row_idx, :, 1],
                                           c="r",
                                           s=1,
                                           alpha=point_alpha)
        if pred_handjoints3dw is not None:
            visualize_joints_2d(axes[row_idx, col_idx],
                                pred_handjoints3dw[row_idx][:],
                                alpha=1,
                                joint_idxs=False)
        if gt_handjoints3dw is not None:
            visualize_joints_2d(axes[row_idx, col_idx],
                                gt_handjoints3dw[row_idx][:],
                                alpha=0.5,
                                joint_idxs=False)
        ...
        if pred_objverts3dw is not None and gt_objverts3dw is not None:
            arrow_nb = 6
            arrows = torch.cat(
                [gt_objverts3dw[:, :arrow_nb], pred_objverts3dw[:, :arrow_nb]],
                1)
            links = [[i, i + arrow_nb] for i in range(arrow_nb)]
            visualize_joints_2d(
                axes[row_idx, col_idx],
        ...
        col_idx = 3
        if gt_objverts3dw is not None:
            axes[row_idx, col_idx].scatter(gt_objverts3dw[row_idx, :, 1],
                                           gt_objverts3dw[row_idx, :, 2],
                                           c="b",
                                           s=1,
                                           alpha=point_alpha)
        if pred_objverts3dw is not None:
            axes[row_idx, col_idx].scatter(pred_objverts3dw[row_idx, :, 1],
                                           pred_objverts3dw[row_idx, :, 2],
                                           c="r",
                                           s=1,
                                           alpha=point_alpha)
        if pred_handjoints3dw is not None:
            visualize_joints_2d(axes[row_idx, col_idx],
                                pred_handjoints3dw[row_idx][:, 1:],
                                alpha=1,
                                joint_idxs=False)
        if gt_handjoints3dw is not None:
            visualize_joints_2d(axes[row_idx, col_idx],
                                gt_handjoints3dw[row_idx][:, 1:],
                                alpha=0.5,
                                joint_idxs=False)
        if pred_objcorners3dw is not None:
            visualize_joints_2d(
        ...
        if pred_objverts3dw is not None and gt_objverts3dw is not None:
            arrow_nb = 6
            arrows = torch.cat([
                gt_objverts3dw[:, :arrow_nb, 1:],
                pred_objverts3dw[:, :arrow_nb, 1:]
            ], 1)
            links = [[i, i + arrow_nb] for i in range(arrow_nb)]
            visualize_joints_2d(
                axes[row_idx, col_idx],
        ...
        if display_centered:
            ...
            if gt_canobjverts3d is not None:
                axes[row_idx, col_idx].scatter(gt_canobjverts3d[row_idx, :, 0],
                                               gt_canobjverts3d[row_idx, :, 1],
                                               c="b",
                                               s=1,
                                               alpha=point_alpha)
            if pred_objverts3d is not None:
                axes[row_idx, col_idx].scatter(pred_objverts3d[row_idx, :, 0],
                                               pred_objverts3d[row_idx, :, 1],
                                               c="r",
                                               s=1,
                                               alpha=point_alpha)
            ...
            if pred_objcorners3d is not None and gt_canobjcorners3d is not None:
                arrow_nb = 6
                arrows = torch.cat([
                    gt_canobjcorners3d[:, :arrow_nb],
                    pred_objcorners3d[:, :arrow_nb]
                ], 1)
                links = [[i, i + arrow_nb] for i in range(arrow_nb)]
                visualize_joints_2d(
                    axes[row_idx, col_idx],
            ...
            if gt_objverts3d is not None:
                axes[row_idx, col_idx].scatter(gt_objverts3d[row_idx, :, 0],
                                               gt_objverts3d[row_idx, :, 1],
                                               c="b",
                                               s=1,
                                               alpha=point_alpha)
            # if pred_objverts3d is not None:
            #     axes[row_idx, 2].scatter(
            #         pred_objverts3d[row_idx, :, 0], pred_objverts3d[row_idx, :, 1], c="r", s=1, alpha=point_alpha
            #     )
            if gt_handverts3d is not None:
                axes[row_idx, col_idx].scatter(gt_handverts3d[row_idx, :, 0],
                                               gt_handverts3d[row_idx, :, 1],
                                               c="g",
                                               s=1,
                                               alpha=0.2)
            if pred_handverts3d is not None:
                axes[row_idx, col_idx].scatter(pred_handverts3d[row_idx, :, 0],
                                               pred_handverts3d[row_idx, :, 1],
                                               c="c",
                                               s=1,
                                               alpha=0.2)
            if pred_handjoints3d is not None:
                visualize_joints_2d(axes[row_idx, col_idx],
                                    pred_handjoints3d[row_idx],
                                    alpha=1,
                                    joint_idxs=False)
            if gt_handjoints3d is not None:
                visualize_joints_2d(axes[row_idx, col_idx],
                                    gt_handjoints3d[row_idx],
                                    alpha=0.5,
                                    joint_idxs=False)
            axes[row_idx, col_idx].invert_yaxis()
            ...
            col_idx = 6
            if gt_objverts3d is not None:
                axes[row_idx, col_idx].scatter(gt_objverts3d[row_idx, :, 1],
                                               gt_objverts3d[row_idx, :, 2],
                                               c="b",
                                               s=1,
                                               alpha=point_alpha)
            # if pred_objverts3d is not None:
            #     axes[row_idx, 3].scatter(
            #         pred_objverts3d[row_idx, :, 1], pred_objverts3d[row_idx, :, 2], c="r", s=1, alpha=point_alpha
            #     )
            if gt_handverts3d is not None:
                axes[row_idx, col_idx].scatter(gt_handverts3d[row_idx, :, 1],
                                               gt_handverts3d[row_idx, :, 2],
                                               c="g",
                                               s=1,
                                               alpha=0.2)
            if pred_handverts3d is not None:
                axes[row_idx, col_idx].scatter(pred_handverts3d[row_idx, :, 1],
                                               pred_handverts3d[row_idx, :, 2],
                                               c="c",
                                               s=1,
                                               alpha=0.2)
            if pred_handjoints3d is not None:
                visualize_joints_2d(axes[row_idx, col_idx],
                                    pred_handjoints3d[row_idx][:, 1:],
                                    alpha=1,
                                    joint_idxs=False)
            if gt_handjoints3d is not None:
                visualize_joints_2d(axes[row_idx, col_idx],
                                    gt_handjoints3d[row_idx][:, 1:],
                                    alpha=0.5,
                                    joint_idxs=False)
