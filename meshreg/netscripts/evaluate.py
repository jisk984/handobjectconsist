...
def recover_back(joints_trans, affinetrans):
    ...
    hom2d = torch.cat(
        [joints_trans,
         joints_trans.new_ones(batch_size, point_nb, 1)], -1)
    rec2d = torch.inverse(affinetrans).bmm(hom2d.transpose(
        1, 2).float()).transpose(1, 2)[:, :, :2]
    return rec2d


def recover_3d_proj(joints3d,
                    joints2d,
                    camintr,
                    est_scale,
                    est_trans,
    ...
    est_XY0 = (est_trans[:, 0] - camintr[:, :2, 2]) * est_Z0[:, 0] / focal[:,
                                                                           0]
...
def feed_avg_meters(avg_meters, sample, results):
    if "obj_verts2d" in results and results[
            "obj_verts2d"] is not None and BaseQueries.OBJVERTS2D in sample:
        ...
        rec_pred = recover_back(results["obj_verts2d"].detach().cpu(),
                                affinetrans)
        ...
        avg_meters.add_loss_value("objverts2d_mepe",
                                  verts2d_dists.mean().item())
    if "obj_verts3d" in results and results[
            "obj_verts3d"] is not None and BaseQueries.OBJVERTS3D in sample:
        recov_objverts3d = results["recov_objverts3d"]
        or_verts3d = sample[BaseQueries.OBJVERTS3D]
        verts3d_diststrans = (results["recov_objverts3d"].cpu() -
                              or_verts3d).norm(2, -1)
        avg_meters.add_loss_value("objverts3d_mepe_trans",
                                  verts3d_diststrans.mean().item())
        verts3d_chamfer = chamfer.chamfer_distance(recov_objverts3d,
                                                   or_verts3d.cuda())[0]
        avg_meters.add_loss_value("objverts3d_chamfer", verts3d_chamfer.item())
...
def feed_evaluators(evaluators, sample, results, idxs=None, center_idx=9):
    ...
    if "joints2d" in results and BaseQueries.JOINTS2D in sample:
        ...
        rec_pred = recover_back(results["joints2d"].detach().cpu(),
                                affinetrans)
        ...
        if gt_err > 1:
            warnings.warn(f"Back to orig error on gt {gt_err} > 1 pixel")
        for gt_joints, pred_joints in zip(rec_pred.numpy(),
                                          or_joints2d.cpu().numpy()):
            evaluators["joints2d_base"].feed(gt_joints, pred_joints)
    # Object 2d metric
    if ("obj_corners2d" in results and results["obj_corners2d"] is not None
        ...
        rec_pred = recover_back(results["obj_corners2d"].detach().cpu(),
                                affinetrans)
        ...
        if gt_err > 1:
            warnings.warn(f"Back to orig error on gt {gt_err} > 1 pixel")
        for gt_corners, pred_corners in zip(rec_pred.numpy(),
                                            or_corners2d.cpu().numpy()):
    ...
    if "joints3d" in results and BaseQueries.JOINTS3D in sample:
        ...
        if center_idx is not None:
            gt_joints3d_cent = gt_joints3d - gt_joints3d[:, center_idx:
                                                         center_idx + 1]
            pred_joints3d_cent = pred_joints3d - pred_joints3d[:, center_idx:
                                                               center_idx + 1]
            for gt_joints, pred_joints in zip(gt_joints3d_cent.numpy(),
                                              pred_joints3d_cent.numpy()):
        ...
        if "recov_joints3d" in results:
            ...
            for gt_joints, pred_joints in zip(
                    sample[BaseQueries.JOINTS3D].cpu(), joints3d_pred):
...
def parse_evaluators(evaluators, config=None):
    ...
    for evaluator_name, evaluator in evaluators.items():
        ...
        (epe_mean, epe_mean_joints, epe_median, auc, pck_curve,
         thresholds) = evaluator.get_measures(start, end, steps)
