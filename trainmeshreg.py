...
def main(args):
    ...
    print(
        f"Saving experiment logs, models, and training curves and images at {exp_id}"
    )
    ...
    for epoch_idx in tqdm(range(epoch, args.epochs), desc="epoch"):
        ...
        if args.evaluate or (args.eval_freq != -1
                             and epoch_idx % args.eval_freq == 0):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--com",
                        default="debug/",
                        help="Prefix for experimental results")
    ...
    parser.add_argument(
        ...
        help=
        "HO3D possible splits, 'paper' for hand baseline, 'objects' for photometric consistency",
    )
    parser.add_argument(
        "--mini_factor",
        type=float,
        default=1,
        help="Work on fraction of the datase for debugging purposes")
    parser.add_argument("--fraction",
                        type=float,
                        default=1,
                        help="Fraction of data with full supervision")
    parser.add_argument("--use_cache", action="store_true")
    parser.add_argument("--no_augm",
                        action="store_true",
                        help="Prevent all data augmenation")
    parser.add_argument("--block_rot",
                        action="store_true",
                        help="Prevent rotation during data augmentation")
    parser.add_argument("--max_rot",
                        default=0,
                        type=float,
    ...
    parser.add_argument("--version",
                        default=3,
                        type=int,
                        help="Version of synthetic HO3D dataset to use")
    parser.add_argument("--center_idx", default=9, type=int)
    parser.add_argument("--center_jittering",
                        type=float,
                        default=0.1,
                        help="Controls magnitude of center jittering")
    parser.add_argument("--scale_jittering",
                        type=float,
                        default=0,
                        help="Controls magnitude of scale jittering")
    ...
    parser.add_argument(
        "--freeze_batchnorm",
        action="store_true",
        help="Freeze batchnorm layer values"
        "to ImageNet initialized values",
    )

    # Training parameters
    parser.add_argument("--evaluate",
                        action="store_true",
                        help="Only evaluation run")
    ...
    parser.add_argument(
        ...
        "factor, if 1, no decay is effectively applied",
    )
    ...
    parser.add_argument("--criterion2d",
                        ...
                        default="l2")
    parser.add_argument("--mano_lambda_verts3d",
                        type=float,
                        default=0,
                        help="Weight for 3D vertices supervision")
    parser.add_argument("--mano_lambda_joints3d",
                        type=float,
                        default=0,
                        help="Weight for 3D joints supervision, centered")
    parser.add_argument(
    ...
    parser.add_argument("--mano_lambda_joints2d",
                        type=float,
                        default=0,
                        help="Weight for 2D joints supervision")
    parser.add_argument("--mano_lambda_verts2d",
                        type=float,
                        default=0,
                        help="Weight for 2D vertices supervision")
    parser.add_argument("--mano_lambda_shape",
                        type=float,
                        default=5e-07,
                        help="Weight for hand shape regularization")
    parser.add_argument("--mano_lambda_pose_reg",
                        type=float,
                        default=5e-06,
    ...
    parser.add_argument("--obj_trans_factor",
                        type=float,
                        default=100,
                        help="Multiplier for translation prediction")
    parser.add_argument("--obj_scale_factor",
                        type=float,
                        default=0.0001,
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="How often to generate visualizations (training steps)")
    parser.add_argument("--epoch_display_freq",
                        type=int,
                        default=1,
                        help="How often to generate visualizations (epochs)")
    parser.add_argument("--snapshot",
                        type=int,
                        default=100,
                        help="How often to save intermediate models (epochs)")
